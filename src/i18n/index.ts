import i18n from "i18next";
import { initReactI18next } from "react-i18next";

i18n.use(initReactI18next).init({
  resources: {
    es: {
      translation: {
        welcome: "Bienvenido",
        login: "Iniciar sesión",
        register: "Registrarse",
        loginFormDescription:
          "Introduzca su correo electrónico debajo para iniciar sesión",
        forgotYourPassword: "¿Olvidó su contraseña?",
        email: "Correo electrónico",
        password: "Contraseña",
        adminPanel: "Panel de Administrador",
        administrator: "Administrador",
        dashboard: "Dashboard",
        orders: "Ordenes",
        users: "Usuarios",
        products: "Productos",
        categories: "Categorías",
        brands: "Marcas",
        materials: "Materiales",
        unitsSold: "Unidades Vendidas",
        revenue: "Ingresos",
        averagePrice: "Precio Promedio",
        stock: "Inventario",
        actions: "Acciones",
        edit: "Editar",
        delete: "Eliminar",
        previous: "Anterior",
        next: "Siguiente",
        page: "Página",
        noResults: "Sin resultados",
        image: "Imágen",
        cancel: "Cancelar",
        areYouSure: "¿Estás seguro?",
        cannotUndo: "Esta accion no se puede revertir",
        store: "Tienda",
        myProfile: "Mi Perfil",
        settings: "Configuración",
        logout: "Cerrar sesión",
        adminMenuOpen: "Abrir menú",
        showPassword: "Mostrar contraseña",
        hidePassword: "Ocultar contraseña",
        searchFor: "Busca",
        upload: "Subir",
        uploadImage: "Subir una imágen",
        uploadImageDescription: "Asignar una imágen debajo",
        imageUploaded: "Imágen subida con éxito",
        imageNotUploaded: "No fue posible subir la imágen",
        categoryDeletedSuccessfully: "Categoría eliminada con éxito",
        categoryNotDeletedSuccessfully: "No fue posible eliminar la categoría",
        notEnoughPermissions: "No cuentas con los permisos para realizar esta acción",
        category: "Categoria",
        editCategoryFormDescription: "Editar el nombre de una categoría existente",
        createCategory: "Crear categoría",
        createCategoryFormDescription: "Crea una nueva categoría para tu sitio",
        categoryCreatedSuccessfully: "Categoría creada con éxito",
        categoryNotCreatedSuccessfully: "No fue posible crear la categoría",
        categoryNotEditedSuccessfully: "No fue posible editar la categoría",
        restoreCategoryTitle: "Categoría eliminada anteriormente",
        restoreCategoryDescription: "Una categoría con este nombre fue eliminada anteriormente. Puedes restaurarla para recuperarla, o usar un nombre diferente.",
        restoreCategory: "Restaurar categoría",
        useDifferentName: "Usar un nombre diferente",
        categoryRestoredSuccessfully: "Categoría restaurada con éxito",
        categoryNotRestoredSuccessfully: "No fue posible restaurar la categoría",
        categoryNotFound: "Categoría no encontrada o eliminada",
        back: "Volver",
        categoryDetails: "Detalles de categoría",
        brand: "Marca",
        slug: "Slug",
        createBrand: "Crear marca",
        createBrandFormDescription: "Crea una nueva marca para tu sitio",
        editBrandFormDescription: "Editar el nombre de una marca existente",
        brandCreatedSuccessfully: "Marca creada con éxito",
        brandNotCreatedSuccessfully: "No fue posible crear la marca",
        brandNotEditedSuccessfully: "No fue posible editar la marca",
        brandDeletedSuccessfully: "Marca eliminada con éxito",
        brandNotDeletedSuccessfully: "No fue posible eliminar la marca",
        restoreBrandTitle: "Marca eliminada anteriormente",
        restoreBrandDescription:
          "Una marca con este nombre fue eliminada anteriormente. Puedes restaurarla para recuperarla, o usar un nombre diferente.",
        restoreBrand: "Restaurar marca",
        brandRestoredSuccessfully: "Marca restaurada con éxito",
        brandNotRestoredSuccessfully: "No fue posible restaurar la marca",
        brandNotFound: "Marca no encontrada o eliminada",
        brandDetails: "Detalles de marca",
        material: "Material",
        createMaterial: "Crear material",
        createMaterialFormDescription: "Crea un nuevo material para tu sitio",
        editMaterialFormDescription: "Editar el nombre de un material existente",
        materialCreatedSuccessfully: "Material creado con éxito",
        materialNotCreatedSuccessfully: "No fue posible crear el material",
        materialNotEditedSuccessfully: "No fue posible editar el material",
        materialDeletedSuccessfully: "Material eliminado con éxito",
        materialNotDeletedSuccessfully: "No fue posible eliminar el material",
        restoreMaterialTitle: "Material eliminado anteriormente",
        restoreMaterialDescription:
          "Un material con este nombre fue eliminado anteriormente. Puedes restaurarlo para recuperarlo, o usar un nombre diferente.",
        restoreMaterial: "Restaurar material",
        materialRestoredSuccessfully: "Material restaurado con éxito",
        materialNotRestoredSuccessfully: "No fue posible restaurar el material",
        materialNotFound: "Material no encontrado o eliminado",
        materialDetails: "Detalles de material",
        quickActions: "Acciones rápidas",
        lowStockWarning: "Stock bajo",
        topProducts: "Mejores productos",
        lowStockProducts: "Productos con poco stock",
        name: "Nombre",
        price: "Precio",
        avgPrice: "Precio prom.",
        avgDiscountPrice: "P. con dto.",
        createProduct: "Crear producto",
        productCreatedSuccessfully: "Producto creado con éxito",
        productNotCreatedSuccessfully: "No fue posible crear el producto",
        productImageUploadFailed: "No fue posible subir la imagen del producto",
        shortDescription: "Descripción corta",
        longDescription: "Descripción larga",
        sku: "SKU",
        discountPrice: "Precio con descuento",
        weightGrams: "Peso (gramos)",
        addVariant: "Agregar variante",
        addAttributeValue: "Agregar valor de atributo",
        selectAttribute: "Seleccionar atributo",
        selectValue: "Seleccionar valor",
        removeVariant: "Eliminar variante",
        active: "Activo",
        inactive: "Inactivo",
        outOfStock: "Agotado",
        optional: "(opcional)",
        add: "Agregar",
        basicInfo: "Información básica",
        categorization: "Categorización",
        variantsSection: "Variantes",
        variant: "Variante",
        attributeValues: "Valores de atributo",
        noAttributeValues: "Sin valores de atributo",
        status: "Estado",
        featured: "Destacado",
        yes: "Sí",
        no: "No",
        all: "Todos",
        sortBy: "Ordenar por",
        sortDirection: "Dirección",
        ascending: "Ascendente",
        descending: "Descendente",
        perPage: "Por página",
        filters: "Filtros",
        clearFilters: "Limpiar filtros",
        comingSoon: "Próximamente",
        description: "Descripción",
        variants: "Variantes",
        createdAt: "Creado",
        updatedAt: "Actualizado",
        notFound: {
          title: "Página no encontrada",
          body: "Lo sentimos, la página que buscas no existe o fue movida.",
          cta: "Volver al inicio",
        },
        categoryPage: {
          empty: "No hay productos en esta categoría todavía.",
        },
        brandPage: {
          empty: "No hay productos de esta marca todavía.",
        },
        categoriesList: {
          title: "Categorías",
          empty: "No hay categorías disponibles por el momento.",
        },
        brandsList: {
          title: "Marcas",
          empty: "No hay marcas disponibles por el momento.",
        },
        auth: {
          invalidCredentials: "Correo electrónico o contraseña incorrectos",
          loginFailed: "No fue posible iniciar sesión. Inténtalo de nuevo.",
          networkError:
            "No pudimos conectar con el servidor. Revisa tu conexión.",
          register: {
            title: "Crear cuenta",
            description: "Completa tus datos para registrarte",
            confirmPasswordLabel: "Confirmar contraseña",
            termsLabel: "Acepto los términos y condiciones",
            submit: "Registrarse",
            success: "Cuenta creada con éxito",
            emailExists: "El correo electrónico ya está registrado",
            failed: "No fue posible completar el registro. Inténtalo de nuevo.",
            passwordMismatch: "Las contraseñas no coinciden",
            termsRequired: "Debes aceptar los términos y condiciones",
            haveAccount: "¿Ya tienes una cuenta?",
          },
          verify: {
            title: "Verifica tu correo electrónico",
            description: "Ingresa el código de 6 dígitos que enviamos a {{email}}",
            codeLabel: "Código de verificación",
            submit: "Verificar",
            resend: "Reenviar código",
            resent: "Código reenviado",
            success: "Cuenta verificada correctamente",
            codeRejected: "El código ingresado es incorrecto",
            codeExpired: "El código ha expirado. Solicita uno nuevo",
            attemptsExceeded:
              "Superaste el número máximo de intentos. Solicita un nuevo código",
            tooManyCodes:
              "Solicitaste demasiados códigos. Inténtalo de nuevo más tarde",
            alreadyVerified: "Esta cuenta ya fue verificada",
            failed: "No fue posible verificar el código. Inténtalo de nuevo.",
            resendFailed:
              "No fue posible reenviar el código. Inténtalo de nuevo.",
          },
        },
        cart: {
          title: "Carrito",
          empty: "Tu carrito está vacío",
          subtotal: "Subtotal",
          checkout: "Finalizar compra",
          viewCart: "Ver carrito",
          remove: "Eliminar",
          quantity: "Cantidad",
          addToCart: "Agregar al carrito",
          added: "Producto agregado al carrito",
          stockExceeded: "No hay suficiente stock disponible",
          continueShopping: "Seguir comprando",
          increase: "Aumentar cantidad",
          decrease: "Disminuir cantidad",
          openCart: "Abrir carrito",
          selectVariant: "Selecciona una opción",
          outOfStock: "Sin stock",
          merged: {
            adjusted:
              "Se ajustó la cantidad de {{count}} artículo(s) de tu carrito de invitado por disponibilidad de stock",
            skipped:
              "{{count}} artículo(s) de tu carrito de invitado ya no están disponibles y no se agregaron",
          },
        },
        order: {
          checkout: {
            title: "Finalizar compra",
            shippingSection: "Dirección de envío",
            recipient: "Destinatario",
            line1: "Dirección",
            line2: "Dirección (línea 2)",
            city: "Ciudad",
            state: "Estado",
            postalCode: "Código postal",
            country: "País",
            phone: "Teléfono",
            summaryTitle: "Resumen del pedido",
            shippingNotice:
              "El costo de envío se calcula al confirmar el pedido",
            submit: "Confirmar pedido",
            emptyCart: "Tu carrito está vacío",
            backToCart: "Volver al carrito",
            genericError:
              "No fue posible completar tu pedido. Inténtalo de nuevo.",
            conflictStockInsufficient:
              "La cantidad se ajustó a la disponibilidad actual ({{available}} disponibles). Revisa e inténtalo de nuevo.",
            conflictProductUnavailable:
              "Este producto ya no está disponible. Elimínalo del carrito para continuar.",
          },
          history: {
            title: "Mis pedidos",
            empty: "Aún no tienes pedidos",
            orderNumber: "Pedido",
            viewDetail: "Ver detalle",
          },
          detail: {
            title: "Detalle del pedido",
            confirmationBanner: "¡Gracias por tu compra!",
            subtotal: "Subtotal",
            shippingCost: "Envío",
            total: "Total",
            items: "Artículos",
            shippingAddress: "Dirección de envío",
            notFound: "Pedido no encontrado",
            backToOrders: "Volver a mis pedidos",
          },
          status: {
            PENDING: "Pendiente",
            CONFIRMED: "Confirmado",
            SHIPPED: "Enviado",
            DELIVERED: "Entregado",
            CANCELLED: "Cancelado",
          },
        },
        account: {
          title: "Mi cuenta",
          sidebar: {
            profile: "Mi perfil",
            orders: "Mis órdenes",
            addresses: "Direcciones",
            wishlist: "Lista de deseos",
            logout: "Cerrar sesión",
          },
          profile: {
            heading: "Datos personales",
            firstName: "Nombre",
            paternalLastName: "Apellido paterno",
            maternalLastName: "Apellido materno",
            phone: "Teléfono",
            email: "Correo electrónico",
            username: "Usuario",
            save: "Guardar cambios",
            saved: "Perfil actualizado",
            loadError: "No fue posible cargar tu perfil",
            photo: {
              heading: "Foto de perfil",
              change: "Cambiar foto",
              choose: "Seleccionar imagen",
              confirm: "Subir foto",
              uploading: "Subiendo...",
              uploaded: "Foto de perfil actualizada",
              invalidType: "Formato de imagen no válido",
              tooLarge: "La imagen es demasiado grande (máximo 5 MB)",
              error: "No fue posible subir la imagen",
              preview: "Vista previa",
            },
          },
          addresses: {
            heading: "Mis direcciones",
            add: "Agregar nueva dirección",
            empty: "Aún no tienes direcciones guardadas",
            defaultBadge: "Predeterminada",
            edit: "Editar",
            delete: "Eliminar",
            makeDefault: "Marcar como predeterminada",
            deleteConfirm: "¿Eliminar esta dirección?",
            created: "Dirección creada",
            updated: "Dirección actualizada",
            deleted: "Dirección eliminada",
            defaultSet: "Dirección predeterminada actualizada",
            loadError: "No fue posible cargar tus direcciones",
            form: {
              street: "Calle",
              exteriorNumber: "Número exterior",
              interiorNumber: "Número interior",
              interiorNumberHint: "si aplica",
              recipientName: "Nombre del destinatario",
              phone: "Teléfono",
              postalCode: "Código postal",
              state: "Estado",
              municipality: "Municipio",
              city: "Ciudad",
              neighborhood: "Colonia",
              neighborhoodOther: "Otra / no aparece mi colonia",
              neighborhoodOtherLabel: "Escribe tu colonia",
              referenceNotes: "Referencias",
              submit: "Guardar dirección",
              cpLookingUp: "Verificando código postal...",
              cpVerified: "Código postal verificado",
              cpUnverifiedWarning:
                "No pudimos verificar tu código postal. Revisa los datos e inténtalo de nuevo.",
            },
          },
        },
        validation: {
          required: "Este campo es obligatorio",
          maxLength80: "Máximo 80 caracteres",
          maxLength120: "Máximo 120 caracteres",
          maxLength150: "Máximo 150 caracteres",
          postalCode: "El código postal debe tener 5 dígitos",
          invalidEmail: "Correo electrónico inválido",
          minLength: "Debe tener al menos 3 caracteres",
          maxLength: "Debe tener como máximo 60 caracteres",
          maxLength20: "Máximo 20 caracteres",
          maxLength30: "Máximo 30 caracteres",
          maxLength100: "Máximo 100 caracteres",
          maxLength200: "Máximo 200 caracteres",
          maxLength500: "Máximo 500 caracteres",
          maxLength5000: "Máximo 5000 caracteres",
          minPrice: "El precio debe ser mayor a 0",
          minStock: "El stock no puede ser negativo",
          minWeight: "El peso debe ser mayor a 0",
          minOneVariant: "Se requiere al menos una variante",
          passwordMin: "La contraseña debe tener al menos 8 caracteres",
          passwordMax: "La contraseña debe tener como máximo 64 caracteres",
          passwordPattern:
            "La contraseña debe contener mayúsculas, minúsculas, número y carácter especial"
        }
      },
    },
  },

  lng: "es",
  fallbackLng: "es",

  interpolation: {
    escapeValue: false,
  },
});

export default i18n;
