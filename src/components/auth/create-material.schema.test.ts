...
import { createMaterialSchema } from "./create-material.schema";

describe("createMaterialSchema", () => {
  it("accepts a name at the minimum boundary (3 chars)", () => {
    const result = createMaterialSchema.safeParse({ name: "Gol" });
    expect(result.success).toBe(true);
  });

  it("accepts a name at the maximum boundary (60 chars)", () => {
    const result = createMaterialSchema.safeParse({ name: "a".repeat(60) });
    expect(result.success).toBe(true);
  });

  it("rejects a name shorter than 3 chars with the minLength key", () => {
    const result = createMaterialSchema.safeParse({ name: "Au" });
  ...
  it("rejects a name longer than 60 chars with the maxLength key", () => {
    const result = createMaterialSchema.safeParse({ name: "a".repeat(61) });
    expect(result.success).toBe(false);
    const issue = result.error?.issues.find((i) => i.path[0] === "name");
