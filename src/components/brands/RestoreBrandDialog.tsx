...
import { restoreBrand } from "@/api/api";
import { useUser } from "@/hooks/useUser";
import { useToast } from "@/hooks/useToast";
import { useApiErrorHandler } from "@/hooks/useApiErrorHandler";

interface RestoreBrandDialogProps {
  open: boolean;
...
export default function RestoreBrandDialog({
  open,
  onOpenChange,
  brandId,
  onRestored,
  onUseDifferentName,
}: RestoreBrandDialogProps) {
  ...
  const { mutate: handleRestore, isPending } = useMutation({
    mutationFn: () => restoreBrand({ brandId, token: user?.token || "" }),
    onSuccess: (data) => {
      success(data?.message || t("brandRestoredSuccessfully"));
      queryClient.invalidateQueries({ queryKey: ["admin", "brands"] });
      onRestored();
    },
    onError: (err: Error) => handleError(err, t("brandNotRestoredSuccessfully")),
  });
